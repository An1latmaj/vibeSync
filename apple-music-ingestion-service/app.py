...
logger = logging.getLogger("apple-music-ingestion-service")
...
async def apple_music_upload(user_id: str = Form(...), file: UploadFile = File(...)):
    ...
    job_row = await ingest_apple_music_file(user_id=user_id, file_obj=file_obj, filename=file.filename or "upload.csv", filesize=len(contents))
